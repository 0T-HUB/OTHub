<script lang="ts">
  ...
  onMount(async () => {
    ...
    const accessTokenCookieDoesNotExist = !document.cookie.includes('accessToken')
    if (accessTokenCookieDoesNotExist) askForToken()
  })

  const askForToken = () => {
    const accessToken = prompt('Please, enter the access token.')
    if (accessToken) {
      document.cookie = `accessToken=${accessToken}; Max-Age=86400`
